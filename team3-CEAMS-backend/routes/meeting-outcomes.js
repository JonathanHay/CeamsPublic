...
router.post('/', function (req, res) {
    ...
    meetingOutcome.save(function (err) {
        if (err) res.status(500).json(err);
        Meetings.Model.findById(req.body.meetingOutcome.meetings, function (err, meeting) {
            if (err) res.status(500).json(err);
        ...
        MeetingOutcomes.Model.findOneAndDelete({ _id: req.params.id },
            function (err, deleted) {
                if (err) res.status(500).json(err);
                    ...
                    Meetings.Model.findById(meetingOutcome.meetings, function (err, meeting) {
                        if (err) res.status(500).json(err);
                        ...
                        meeting.save(function (err) {
                            if (err) res.status(500).json(err);
                            res.json({ deleted: meetingOutcome });
                        })
                    })
                }
            }
